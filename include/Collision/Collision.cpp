...
bool CircleCollider::Collision(Vec v1, Vec v2) {
	Vec circleCenter(col.x, col.y);
	float gaiseki = Vec::GetCrossVec2(v2 - v1, circleCenter-v1);
	if (gaiseki < 0) { gaiseki *= -1; }
	float dist = gaiseki / Vec::GetVecLen(v2 - v1);
	if (dist > col.r) { return false; }
	float dot1 = Vec::GetDotVec(circleCenter - v1, v2 - v1);
	float dot2 = Vec::GetDotVec(circleCenter - v2, v2 - v1);
	if (dot1*dot2 <= 0) { return true; }
	float len1 = Vec::GetVecLen(circleCenter - v1);
	float len2 = Vec::GetVecLen(circleCenter - v2);
	if (col.r > len1 || col.r > len2) { return true; }
	return false;
}
...
bool CircleCollider::Collision(Circle c) {
	Vec center(col.x, col.y);
	Vec center2(c.x, c.y);
	float dist = Vec::GetVecLen(center2 - center);
	if (dist <= (col.r + c.r)) { return true; }
	return false;
}
